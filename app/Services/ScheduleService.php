...
class ScheduleService
{
    ...
    public function getActiveSchedule(): ?Schedule
    {
        return Cache::remember(
            ...
            self::CACHE_TTL,
            function () {
                return Schedule::with('availability.slots', 'days.availability.slots')
                    ->where('active', true)
                    ->first()
                    ?? Schedule::with('availability.slots', 'days.availability.slots')
    ...
    protected function getAvailableTimeslotsForWeeklySchedule(
        ...
        $scheduleDays = $activeSchedule->days->keyBy(fn ($day) => $day->type->value);

        return $this->processDateRange(
            $dateRange->get('start'),
            $dateRange->get('end'),
            function (Carbon $currentDate) use ($scheduleDays, $bookedAppointments) {
                ...
                $scheduleDay = $scheduleDays->get($dayOfWeek);

                return $this->getAvailableSlotsForDay(
                    $currentDate,
                    $this->formatAvailableSlots($scheduleDay->availability->slots),
                    $bookedAppointments
                );
            }
        );
    ...
    protected function getAvailableTimeSlotsForCustomSchedule(
        ...
        $startDate = max($dateRange->get('start'), $activeSchedule->active_from);
        $endDate = min($dateRange->get('end'), $activeSchedule->active_to);
    ...
    protected function getAvailableSlotsForDay(
        ...
        $bookedSlotsForDay = $bookedAppointments->get($dateString, collect());

        $availableSlotsForDay = $availableSlots->reject(
            fn ($slot) => $bookedSlotsForDay->contains($slot)
        );

        return $availableSlotsForDay->isEmpty() ? null : $availableSlotsForDay;
    }

    protected function calculateDateRange(Carbon $date): Collection
    {
        $startDate = Carbon::today()->startOfDay()
            ->addDays(3); //to give 2 days space between current day and nearest available appointment to book
